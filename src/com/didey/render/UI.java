...
public class UI {
	...
	public static void renderHealth(GameContainer gc, Graphics g) {
		...
		// These are fine, as an offset of one pixel will be pretty much the
		// same on all displays.
		g.fillRect(healthBarX + 1, healthBarY + 1,
				((SharedConstants.healthBarWidth) / 100.0f) * Game.player.getMaxHealth() - 1,
				SharedConstants.healthBarHeight - 1);

		g.setColor(Color.white);
		String hpString = "HP: " + Float.toString((Game.player.getMaxHealth()));
	...
	public static void renderDebug(GameContainer gc, Graphics g) {
		...
		g.drawString("MOUSEX: " + Float.toString(gc.getInput().getMouseX()), debugX,
				debugY + (gc.getHeight() / 64.0f) * 2);
		g.drawString("MOUSEY: " + Float.toString(gc.getInput().getMouseY()), debugX,
				debugY + (gc.getHeight() / 64.0f) * 3);
		g.drawString("CAMERA X: " + Float.toString(Game.cameraHandler.getCoords().x), debugX,
				debugY + (gc.getHeight() / 64.0f) * 4);
		g.drawString("CAMERA Y: " + Float.toString(Game.cameraHandler.getCoords().y), debugX,
				debugY + (gc.getHeight() / 64.0f) * 5);	
